...
async def chat_endpoint(request: Request):
  ...
  try:
    ...
    if not user_message:
      return {"success": False, "reply": "No message provided."}

    # Use ChatManager to handle the message
    result = chat_manager.handle_message(user_message)
    
    # Update global state from ChatManager
    current_project_info = chat_manager.get_current_project_info()
    chat_context = chat_manager.chat_context
    
    # Handle edit intents that need project_id
    if result.get("action") == "modify_via_prompt" and current_project_info.get("id"):
      result["project_id"] = current_project_info["id"]
      result["message"] = user_message
    
    # Handle start_generation with project_id for edits
    if result.get("action") == "start_generation" and result.get("is_edit") and current_project_info.get("id"):
      result["project_id"] = current_project_info["id"]
